    ...
    <section ref={sectionRef} id="brands" data-offscreen="false" className="relative bg-[#f6f6f4] px-6 pt-[4vh] pb-[1vh] sm:pb-[4vh]">
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className="marquee marquee-mask overflow-hidden">
          ...
          <div className="marquee-track items-center gap-2 sm:gap-6 md:gap-10">
            {row.map((b, i) => (
              <div key={i} className="flex-none px-1 sm:px-2">
                {/* glass chip holding the official logo — smaller on phones only */}
                <div className="glass group flex h-[56px] w-[92px] items-center justify-center rounded-lg px-3 transition-transform duration-500 hover:-translate-y-1 sm:h-[104px] sm:w-[188px] sm:rounded-2xl sm:px-7">
                  <img
                    src={b.logo}
                    alt={`${b.name} logo`}
                    className="max-h-[22px] w-auto max-w-full object-contain opacity-70 grayscale transition duration-500 group-hover:opacity-100 group-hover:grayscale-0 sm:max-h-[42px]"
                  />
                </div>
              </div>
            ))}
