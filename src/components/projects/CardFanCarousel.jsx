...
/**
 * Card-fan carousel (adapted from a 21st.dev/shadcn TSX component into this
 * project's JSX). GSAP fans the cards; hover nudges neighbours; arrows/dots
 * paginate when there are more than MAX_VISIBLE. Clicking a card calls
 * onCardClick(index) so the parent can open the lightbox (expand).
 *
 * The card sizing/centering lives in index.css (.fan-layout / .fan-card):
 * cards are centred with margin:auto and only moved via GSAP transforms.
 */
...
function getHeightMultiplier(width) {
  ...
  else if (width < 640) idealPx = 19 * 16;
  else if (width < 768) idealPx = 21 * 16;
  else if (width < 1024) idealPx = 25 * 16;
  else idealPx = 29 * 16;
...
export default function CardFanCarousel({ cards, onCardClick }) {
  ...
  useEffect(() => {
    ...
    // hover interactions (skip on reduced motion / touch)
    if (reduce) return;
  ...
  return (
    <section className="relative z-20 flex w-full flex-col items-center px-4 py-0 md:px-8">
        ...
        <div ref={containerRef} className="fan-layout relative flex w-full max-w-[80rem] items-center justify-center">
          {cards.map((card, index) => (
            <button
              ...
              aria-label={card.alt || `View installation ${index + 1}`}
              className="fan-card block cursor-pointer"
            >
              <div className="relative h-full w-full overflow-hidden">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 z-10 h-full w-full object-cover" />
              </div>
            </button>
          ))}
        </div>
      ...
      {needsPagination && (
        <div className="z-30 mt-3 flex items-center justify-center gap-4 md:mt-4">
          <button className={`${ARROW_CLASSES} h-10 w-10 md:h-12 md:w-12`} onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`h-2 w-2 rounded-full transition-all duration-300 ${i === centerIndex ? "scale-[1.3] bg-[#ff2e2e]" : "bg-black/15"}`} />
            ))}
          </div>
